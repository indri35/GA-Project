...
--
-- Host: 127.0.0.1
-- Generation Time: Jan 28, 2017 at 01:42 PM
...
INSERT INTO `users` (`id`, `name`, `email`, `img`, `active_app`, `role`, `password`, `remember_token`, `created_at`, `updated_at`, `status`, `plan`) VALUES
(1, 'Hendrik', 'drikdoank@gmail.com', 'user2-160x160.jpg', '1', 'admin', '$2y$10$rFCavDebMnD4B3CAvG0ptuuj6PaBLUGjVoMXfBk3bj3W8RAHvVkZG', 'EFCsBQre469XjATDZSK4xXCQenQTMVvgXkeVlUDfgDdWl7HRTvNVEa5GP8O8', '2016-09-17 03:02:01', '2017-01-07 23:07:35', '1', 3),
(2, 'Indriyani', 'indri@gmail.com', 'user7-128x128.jpg', '2', 'partner', '$2y$10$avEe2V5oowSLzBkazFliXOud3urBM9ItlugDteFdXLKKQVTFHqQJK', NULL, '2016-09-23 23:37:50', '2016-09-23 23:37:50', '1', 3),
(3, 'Hendra', 'hendrik@gmail.com', 'user2-160x160.jpg', '7', 'partner', 'c3ec0f7b054e729c5a716c8125839829', 'I0IdI1Z05VTSkaC88UIU9inheOBfoCBMSPVSD6s0hN5OnJBlJ4QCfz1Ovixy', '2016-09-26 18:41:32', '2016-09-26 18:41:38', '1', 3),
(4, 'Budi', 'budi@gmail.com', 'user2-160x160.jpg', '3', 'partner', '$2y$10$vHMGDG.id0.dl0etknvCC.xwmwDfRc3K51N2MklH5VITH8Y3eUsnW', 'WGSPz4juSA36OYLskpHDBpzpXN0OPOks5XZs8pQzhxOabF6GJ6ATvDfosolh', '2016-09-26 18:42:01', '2016-09-26 18:42:04', '1', 3),
(5, 'Anis', 'anis@gmail.com', 'user2-160x160.jpg', '5', 'partner', '$2y$10$Vx7.Ba16euhEsXShotUEeOwFOdtjsz22V0/QgkHSLNFG8ZFP89.c6', NULL, '2017-09-26 18:42:47', '2016-09-26 18:42:47', '1', 3),
(6, 'Indriyani', 'indriyani.cs49@gmail.com', 'user7-128x128.jpg', '6', 'partner', '$2y$10$Z/CL3nOYV5Q.FM0fSHFRtuqEcE/IDzH9eb7ItbSx9/9.tmzY8cJuS', 'XvvOH3s6yrHKJXjS2e7yMBoQtFCEkVdYxuMfwEQUsktR9hsLbKLVPEcKYaaE', '2016-10-02 07:14:30', '2017-01-07 23:51:28', '1', 3),
(7, 'Budi', 'drikdoank49@gmail.com', 'user2-160x160.jpg', '13', 'partner', '$2y$10$SRDzbpY6O/r4AA6ooD2YNekNoPkrwD7OqpBzovblsbM.uGgZSsmpC', NULL, '2016-12-22 03:47:44', '2016-12-22 03:47:44', '1', 3);
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `count_connectedby_day` AS select date_format(`master_data`.`created_at`,'%d-%b-%Y') AS `day`,count((case when (`master_data`.`connected_by` = 'wifi') then 1 end)) AS `wifi_count`,count((case when (`master_data`.`connected_by` = 'cellular') then 1 end)) AS `cellular_count` from `master_data` group by `master_data`.`created_at`;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `count_connectedby_day_user` AS select `master_data`.`user` AS `user`,`master_data`.`id_aplikasi` AS `id_aplikasi`,date_format(`master_data`.`created_at`,'%d-%b-%Y') AS `day`,count((case when (`master_data`.`connected_by` = 'wifi') then 1 end)) AS `wifi_count`,count((case when (`master_data`.`connected_by` = 'cellular') then 1 end)) AS `cellular_count` from `master_data` group by `master_data`.`created_at`,`master_data`.`user`,`master_data`.`created_at`,`master_data`.`id_aplikasi`;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `operator_per_day` AS select date_format(`master_data`.`created_at`,'%d-%b-%Y') AS `day`,count((case when (`master_data`.`operator` = 'Telkomsel') then 1 end)) AS `telkomsel_count`,count((case when (`master_data`.`operator` = 'Indosat') then 1 end)) AS `indosat_count`,count((case when (`master_data`.`operator` = 'XL') then 1 end)) AS `xl_count`,count((case when (`master_data`.`operator` = 'Smartfren') then 1 end)) AS `smartfren_count`,count((case when ((`master_data`.`operator` <> 'Telkomsel') and (`master_data`.`operator` <> 'Indosat') and (`master_data`.`operator` <> 'XL') and (`master_data`.`operator` <> 'Smartfren')) then 1 end)) AS `other_count` from `master_data` group by `master_data`.`created_at`;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_activity_by_type_device_per_day` AS select date_format(`master_data`.`created_at`,'%d-%b-%Y') AS `day`,count((case when (`master_data`.`type_device` = 'samsung') then 1 end)) AS `samsung_count`,count((case when (`master_data`.`type_device` = 'apple') then 1 end)) AS `apple_count`,count((case when ((`master_data`.`type_device` <> 'apple') and (`master_data`.`type_device` <> 'samsung')) then 1 end)) AS `other_count` from `master_data` group by `master_data`.`created_at`;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_activity_by_type_device_per_day_user` AS select `master_data`.`user` AS `user`,`master_data`.`id_aplikasi` AS `id_aplikasi`,date_format(`master_data`.`created_at`,'%d-%b-%Y') AS `day`,count((case when (`master_data`.`type_device` = 'samsung') then 1 end)) AS `samsung_count`,count((case when (`master_data`.`type_device` = 'apple') then 1 end)) AS `apple_count`,count((case when ((`master_data`.`type_device` <> 'apple') and (`master_data`.`type_device` <> 'samsung')) then 1 end)) AS `other_count` from `master_data` group by `master_data`.`created_at`,`master_data`.`user`,`master_data`.`id_aplikasi`;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_by_click_per_day_user` AS select `master_data`.`user` AS `user`,`master_data`.`id_aplikasi` AS `id_aplikasi`,date_format(`master_data`.`created_at`,'%d-%b-%Y') AS `day`,count((case when (`master_data`.`click` = 'login') then 1 end)) AS `login_count`,count((case when (`master_data`.`click` = 'save') then 1 end)) AS `save_count`,count((case when (`master_data`.`click` = 'close') then 1 end)) AS `close_count`,count((case when (`master_data`.`click` = 'delete') then 1 end)) AS `delete_count`,count((case when ((`master_data`.`click` <> 'login') and (`master_data`.`click` <> 'save') and (`master_data`.`click` <> 'close') and (`master_data`.`click` <> 'delete')) then 1 end)) AS `other_count` from `master_data` group by `master_data`.`created_at`,`master_data`.`user`,`master_data`.`id_aplikasi`;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_by_view_page_perday_user` AS select `master_data`.`user` AS `user`,`master_data`.`id_aplikasi` AS `id_aplikasi`,date_format(`master_data`.`created_at`,'%d-%b-%Y') AS `day`,count((case when (`master_data`.`view` = 'welcome') then 1 end)) AS `welcome_count`,count((case when (`master_data`.`view` = 'registration') then 1 end)) AS `registration_count`,count((case when (`master_data`.`view` = 'shopping') then 1 end)) AS `shopping_count`,count((case when (`master_data`.`view` = 'order') then 1 end)) AS `order_count`,count((case when ((`master_data`.`view` <> 'welcome') and (`master_data`.`view` <> 'registration') and (`master_data`.`view` <> 'shopping') and (`master_data`.`view` <> 'order')) then 1 end)) AS `other_count` from `master_data` group by `master_data`.`created_at`,`master_data`.`user`,`master_data`.`id_aplikasi`;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_install_per_day` AS select `aplikasi`.`user` AS `user`,date_format(`aplikasi`.`created_at`,'%d-%b-%Y') AS `day`,count('created_at') AS `count_install` from `aplikasi` group by date_format(`aplikasi`.`created_at`,'%d-%b-%Y'),`aplikasi`.`user`;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_install_per_day_admin` AS select date_format(`aplikasi`.`created_at`,'%d-%b-%Y') AS `day`,count('created_at') AS `count_install` from `aplikasi` group by date_format(`aplikasi`.`created_at`,'%d-%b-%Y');
