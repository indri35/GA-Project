...
--
-- Host: 127.0.0.1
-- Generation Time: Apr 21, 2017 at 04:16 PM
...
(6, 'Indriyani', 'indriyani.cs49@gmail.com', 'user7-128x128.jpg', '6', 'partner', '$2y$10$Z/CL3nOYV5Q.FM0fSHFRtuqEcE/IDzH9eb7ItbSx9/9.tmzY8cJuS', 'erbzVPAPyaEp0KcRKw4h1zygXS353OWFbETaDf4v0HtW60nXeAB2vLOXAc3f', '2016-10-02 07:14:30', '2017-04-21 07:01:34', '1', 3, 1),
(11, 'Admin', 'dashboard@appxoffer.com', '/upload/images/profil/11-Hendrik-hhh.jpg', '3', 'admin', '$2y$10$btO9.IxrediwM6YXP489XuLjH//rqC2nG1QTQmclJvWXXtmrah14K', 'm5m6h2vlrYlx5hxSLYi2mEQts3lFfDOFnERRQOIO49FNWQ0cN3vviHDIOae2', '2017-02-12 02:57:38', '2017-04-07 05:09:09', '1', 3, 1),
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_install_month_admin` AS select month(`master_data`.`created_at`) AS `month`,count(distinct `master_data`.`imei`) AS `count_install` from `master_data` group by month(`master_data`.`created_at`);
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_install_per_day` AS select `master_data`.`user` AS `user`,`master_data`.`id_aplikasi` AS `id_aplikasi`,cast(`master_data`.`created_at` as date) AS `day`,count(distinct `master_data`.`imei`) AS `count_install` from `master_data` group by cast(`master_data`.`created_at` as date),`master_data`.`user`,`master_data`.`id_aplikasi`;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_install_per_day_admin` AS select cast(`master_data`.`created_at` as date) AS `day`,count(distinct `master_data`.`imei`) AS `count_install` from `master_data` group by cast(`master_data`.`created_at` as date);
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_install_per_month` AS select `master_data`.`user` AS `user`,`master_data`.`id_aplikasi` AS `id_aplikasi`,month(`master_data`.`created_at`) AS `month`,count(distinct `master_data`.`imei`) AS `count_install` from `master_data` group by month(`master_data`.`created_at`),`master_data`.`user`,`master_data`.`id_aplikasi`;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_install_per_year` AS select `master_data`.`user` AS `user`,`master_data`.`id_aplikasi` AS `id_aplikasi`,year(`master_data`.`created_at`) AS `year`,count(distinct `master_data`.`imei`) AS `count_install` from `master_data` group by year(`master_data`.`created_at`),`master_data`.`user`,`master_data`.`id_aplikasi`;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_install_year_admin` AS select year(`master_data`.`created_at`) AS `year`,count(distinct `master_data`.`imei`) AS `count_install` from `master_data` group by year(`master_data`.`created_at`);
